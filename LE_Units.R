...
flags <- c('Republic_Day','Labour_Day',
           'Independence_Day','Vinayaka_Chaturthi','Dussehra','Diwali')

marcoms <- c('Affiliates_U','Search_L','Affiliates_L','Search_U',
             'Workshops','TV_Marketing','Others')

pricing <- c('Price_Online')

model_vars <- c('Fiscal_Week','Units',pricing, marcoms,flags)
model_vars

first_week <-'FY16 Q1 W01'
...
#Feature Engineering
input_df$Price_Online <- ifelse(input_df$Price_Online > 0 , log(input_df$Price_Online),0)
#input_df$Margin_Online <- ifelse(input_df$Margin_Online > 0 , log(input_df$Margin_Online),0)
